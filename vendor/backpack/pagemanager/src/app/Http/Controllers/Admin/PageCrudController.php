...
//                                ]);
    ...
    { 
        if(\Request::segment(4)=='terms_of_use' ||\Request::segment(4)=='disclaimer' ||\Request::segment(4)=='informed_consent'){
            $this->addDefaultFooterPageFields($template);
        } else {           
    ...
    {         // if the template in the GET parameter is missing, figure it out from the db
        ...
        if($template=='terms_of_use' || $template=='disclaimer' || $template=='informed_consent') { 
            $this->addDefaultFooterPageFields($template); 
        } else { 
            $this->addDefaultPageFields($template);
        }   
    ...
    {
        $model = $this->crud->model;
        $this->data['entry'] = $model::findOrFail($request->id);
        $template = $this->data['entry']->template;
        if($template=='terms_of_use' || $template=='disclaimer' || $template=='informed_consent') {
            $this->addDefaultFooterPageFields($template); 
        } else { 
    ...
    public function addDefaultFooterPageFields($template = false){ 
    ...
    public function addDefaultPageFields($template = false){ 
       ...
       if($template!='terms_of_use' && $template!='disclaimer' && $template!='informed_consent') {
        ...
        $this->crud->addField([
                                ...
                                'label' => 'Banner Type',
                                'type' => 'banner_type',
                                'options' => ['hero_image' => 'Hero Image', 'slider' => 'Slider'], 
                            ]);
        $this->crud->addField([
                                'name' => 'banner_title',
                                'label' => 'Banner title',
