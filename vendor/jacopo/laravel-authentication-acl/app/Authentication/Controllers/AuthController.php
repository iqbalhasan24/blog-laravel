...
class AuthController extends Controller {
    ...
    public function __construct(ReminderService $reminder, ReminderValidator $reminder_validator, AuthenticateInterface $auth) {
    ...
    public function postAdminLogin(Request $request) { 
        ...
        $path = 'page/home';
        if(Session::has('path_name')) {
            $path = 'page/'.Session::get('path_name');
        }
        
        $logged_user = $this->auth->getLoggedUser();
        $data = $logged_user->user_profile()->first();
        if ($logged_user->groups()->first()->id == 1 || $logged_user->groups()->first()->id == 5){  //die(\Session::get('path_name'));
            // $this->setActivitiesLog("logIn", $this->auth->getLoggedUser(), "A user is logs in");
            // return redirect()->route('home');

            return redirect($path);
        }
        elseif (!empty($data->company_name) || !empty($data->logos) || !empty($data->urls) || !empty($data->copay_statement) || !empty($data->monikers)) {
           // $this->setActivitiesLog("logIn", $this->auth->getLoggedUser(), "A user is logs in");
           // return redirect()->route('dashboard.default');
            return redirect($path);
        }
        if ($logged_user->groups()->first()->id == 8 || $logged_user->groups()->first()->id == 9 || $logged_user->groups()->first()->id == 10) {
          //  $this->setActivitiesLog("logIn", $this->auth->getLoggedUser(), "A user is logs in");
            return redirect()->route('contents.list');
        } else {
         //   $this->setActivitiesLog("logIn", $this->auth->getLoggedUser(), "A user is logs in");
            return redirect()->route('users.companyProfile');
        }
    }
    ...
    public function getLogout() {
      //  $this->setActivitiesLog("logOut", $this->auth->getLoggedUser(), "A user is logs out");
    ...
    private function setActivitiesLog($log_keyword, $performedOn, $message) {
        $logged_user = $this->auth->getLoggedUser();
