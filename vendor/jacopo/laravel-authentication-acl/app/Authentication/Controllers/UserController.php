...
class UserController extends Controller {
    ...
    public function getList(Request $request) {
        $group_name = strtolower($this->auth->getLoggedUser()->groups()->first()->name);
        ...
        } elseif ($group_name == "health system admin") {
            $groupId = $this->group_repository->findGroupByName('Health System');
            $group = $this->group_repository->find($groupId->id);
            $users = $group->users;
            return View::make('laravel-authentication-acl::admin.user.list')->with(["users" => $users, "request" => $request]);
        } elseif ($group_name == "health plan admin") {
            $groupId = $this->group_repository->findGroupByName('Health Plan');
            $group = $this->group_repository->find($groupId->id);
            $users = $group->users;
            return View::make('laravel-authentication-acl::admin.user.list')->with(["users" => $users, "request" => $request]);
        } elseif ($group_name == "employer admin") {
            $groupId = $this->group_repository->findGroupByName('Employer');
            $group = $this->group_repository->find($groupId->id);
            $users = $group->users;
            return View::make('laravel-authentication-acl::admin.user.list')->with(["users" => $users, "request" => $request]);
        } else {
            $users = User::where('id', $this->auth->getLoggedUser()->id)->get();
            return View::make('laravel-authentication-acl::admin.user.list-self')->with(["users" => $users, "request" => $request]);
        }
    }

    public function getListForPermission(Request $request) {
        $group_name = strtolower($this->auth->getLoggedUser()->groups()->first()->name);        
        if ($group_name == "admin" || $group_name == "superadmin") {
            $users = $this->user_repository->all($request->except(['page']));
            return View::make('laravel-authentication-acl::admin.user.list-for-permission')->with(["users" => $users, "request" => $request]);
        } 
    }

    public function getPageList(Request $request) {
        $data = array();
        $data['pages'] = Page::select(['template','id'])->groupBy('template')->orderBy('id')->get();
        $data['own_pages'] = UserPagePermission::where('user_id',$request->user_id)->get();

        echo json_encode($data);
    }

    public function addorRemovePagePermission(Request $request) { 
        $page_permission = UserPagePermission::where(['page_id'=>$request->page_id,'user_id'=>$request->user_id])->first();
        
        $msg = 0;
        if(sizeof($page_permission)==0) {
            $page = new UserPagePermission();
           ...
           UserPagePermission::destroy($page_permission->id); 
           $msg = 2;
        }
    ...
    public function postEditUser(Request $request) {
        ...
        if ($this->auth->getLoggedUser()->groups()->first()->name == "Health System Admin") {
            $group = $this->group_repository->findGroupByName('Health System');
            $this->user_repository->addGroup($user->id, $group->id);
        } elseif ($this->auth->getLoggedUser()->groups()->first()->name == "Health Plan Admin") {
            $group = $this->group_repository->findGroupByName('Health Plan');
            $this->user_repository->addGroup($user->id, $group->id);
        } elseif ($this->auth->getLoggedUser()->groups()->first()->name == "Employer Admin") {
            $group = $this->group_repository->findGroupByName('Employer');
            $this->user_repository->addGroup($user->id, $group->id);
        }
