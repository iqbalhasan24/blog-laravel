...
class WelcomeController extends Controller {
    ...
    public function getPage($slug) {
        ...
        $sliders = Article::where([['category_id', $page->id], ['status', 'PUBLISHED']])->get(); 
        $home = Article::where([['category_id', $page->id], ['status', 'PUBLISHED']])->get();

        $user_permissions = UserPagePermission::where(['user_id' => $user->id,'page_id' => $page->id])->count();

        $group_name = strtolower($this->auth->getLoggedUser()->groups()->first()->name);   
        
        //$group_name !="superadmin"
        if($user_permissions==0 && $user->email != "admin@admin.com") {
            return view('admin.pages.not_permission_page');
        }
        ...
        } elseif ($page->template=='partnership') {        

            return View::make('admin.pages.partnership')->with(['user' => $user, 'page' => $page->withFakes(), 'sliders' => $sliders]);

        } elseif ($page->template=='westcon_Comstor_AWS_Cloud_Portfolio') {

            return View::make('admin.pages.portfolio')->with(['user_data' => $user, 'page' => $page->withFakes(), 'portfolio_rows' => $sliders,'sliders' => $sliders ]);
            
        } elseif ($page->template=='training_Enablement') {

            return View::make('admin.pages.training')->with(['user' => $user, 'page' => $page->withFakes(), 'sliders' => $sliders]);
            
        } elseif ($page->template=='engaging_End_Customers') {

            return View::make('admin.pages.engaging')->with(['user' => $user, 'page' => $page->withFakes(), 'sliders' => $sliders]);
            
        } elseif ($page->template=='activition_On_Boarding_And_Support') {

            return View::make('admin.pages.support')->with(['user' => $user, 'page' => $page->withFakes(),'sliders' => $sliders]);
            
        } else {
            return View::make('admin.pages.common_template')->with(['user' => $user, 'page' => $page->withFakes(), 'sliders' => $sliders]); 
        }
    ...
    public function getFooterPage($slug) {
        $page = Page::findBySlugOrId($slug); 
        return View::make('admin.pages.footer-page')->with('page',$page); 
    }
    ...
    public function getWecomePage() {
        ...
        $home = Article::where([['category_id', 10], ['status', 'PUBLISHED']])->get();
